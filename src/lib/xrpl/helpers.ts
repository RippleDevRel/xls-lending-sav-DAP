...
export function getRoleWallet(session: SessionLike, role: WalletInfo["role"]): Wallet {
  ...
  return walletFromSeed(data.seed);
}
...
function roundUp(value: number, isToken: boolean): string {
...
export async function computeFullPaymentAmount(
  ...
  return { amount: roundUp(totalDue + residualBuffer(isToken), isToken), node };
}
...
export async function computeLatePaymentAmount(
  ...
  return { amount: roundUp(totalDue + residualBuffer(isToken), isToken) };
}
